...
import { Member, Task, StoreItem, Redemption, Transaction, Dream, TaskCompletion, DreamStep } from './types';
...
type View = 'role' | 'child_dash' | 'parent_dash' | 'dream_gallery' | 'dream_details' | 'add_dream' | 'tasks' | 'store' | 'achievements' | 'add_task' | 'profile' | 'request_mission' | 'review_dream' | 'add_store_item' | 'wallet' | 'add_member' | 'council_room' | 'journey' | 'map_editor';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const tables = ['members', 'dreams', 'dream_steps', 'tasks', 'task_completions', 'achievements', 'redemptions', 'history', 'store_items'];
    ...
    const handleSaveMap = async (steps: DreamStep[]) => {
        if (!activeMember || !selectedDreamId) return;
    ...
    return (
        <div className="max-w-md mx-auto min-h-screen relative overflow-x-hidden flex flex-col bg-slate-50">
            ...
            {view === 'add_member' && <AddMember onSave={async (m) => { await upsertMember({...m, id: Math.random().toString(36).substr(2, 9), level: 1, xp: 0, coins: 0, dreams: [], tasks: [], taskCompletions: [], achievements: [], redemptions: [], history: [], notifications: {tasks: true, achievements: true}}); setView('role'); }} onBack={() => setView('role')} />}
            ...
            {activeMember && (
                <>
                    ...
                    {view === 'parent_dash' && <ParentDashboard activeParent={activeMember} members={members} onApprove={approveTask} onLogout={handleLogout} onAddTask={() => setView('add_task')} onAddStoreItem={() => setView('add_store_item')} onOpenCouncil={() => setView('council_room')} onPlay={() => setView('child_dash')} onEditMap={(id) => { setSelectedDreamId(id); setView('map_editor'); }} />}
                    {view === 'map_editor' && selectedDream && <MapEditor dream={selectedDream} onSave={handleSaveMap} onBack={() => setView('parent_dash')} />}
                    ...
                    {view === 'add_dream' && <AddDream onAdd={(d) => { updateMemberById(activeMember.id, m => ({...m, dreams: [...m.dreams, {...d, id: Math.random().toString(36).substr(2, 9), currentAmount: 0, status: 'active', steps: []}]})); setView('dream_gallery'); }} onBack={() => setView('dream_gallery')} />}

...
const RoleSelection: React.FC<Props> = ({ members, onSelect }) => {
    ...
    return (
        <div className="flex-1 flex flex-col items-center pt-20 px-8 relative min-h-screen">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-12 w-full max-w-xs">
                {members.map(member => (
                ...
                <div className="flex flex-col items-center">
                    <button className="w-full aspect-square bg-white rounded-[2rem] shadow-sm border border-slate-100 flex items-center justify-center active:scale-95 transition-all">
            ...
            {/* PIN Input Overlay (Estilo da Imagem) */}
            {showPinOverlay && (
                <div className="absolute bottom-10 left-0 right-0 px-6 flex flex-col items-center">
                    <div className="w-full max-w-sm bg-white/60 backdrop-blur-xl rounded-[3rem] p-10 shadow-2xl border-2 border-white flex flex-col items-center">
                        ...
                        {/* Teclado numérico invisível para o usuário digitar */}
